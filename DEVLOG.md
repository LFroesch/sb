...
- Enum/ref-style fields now also open in that overlay and show the relevant choice set there: permissions, launch mode, engine type, prompt refs, engine refs, and hook-bundle refs all surface their valid values in-context, while multiline prompt/hook editors now include example JSON/text shapes in the popup.
- Added and updated regressions in [`internal/tui/update_agent_test.go`](internal/tui/update_agent_test.go) and [`internal/tui/view_agent_test.go`](internal/tui/view_agent_test.go) to lock the new overlay flow and keep short-terminal rendering bounded. Updated [`README.md`](README.md) and [`WORK.md`](WORK.md) to describe the new picker + popup model.
- Why: the awkwardness was no longer just "hook JSON is ugly"; the real issue was that the split-pane setup screen was trying to be both navigator and editor at once. The popup editor separates those jobs and makes the structure feel much more intentional.

### 2026-05-04 — Hook bundles now default to a structured overlay editor instead of raw JSON arrays
- Extended the Advanced Setup overlay flow in [`internal/tui/update_agent_manage.go`](internal/tui/update_agent_manage.go) and [`internal/tui/view_agent_manage.go`](internal/tui/view_agent_manage.go) so hook-bundle `Prompt hooks`, `Pre-shell hooks`, and `Post-shell hooks` no longer drop you into a raw JSON textarea by default. They now open a dedicated hook editor overlay with hook rows on the left and per-hook fields on the right.
- Prompt-hook rows now expose `kind`, `placement`, `label`, `body`, and `body_ref` as normal fields; shell-hook rows expose `name`, `cmd`, `cwd`, `timeout`, `preview_cmd`, and `preview_safe`. Enum/bool fields use the same choice-aware overlay path as the rest of Advanced Setup, and text/multiline fields reuse the popup editors instead of forcing hand-edited arrays.
- Added focused regressions in [`internal/tui/update_agent_test.go`](internal/tui/update_agent_test.go) and [`internal/tui/view_agent_test.go`](internal/tui/view_agent_test.go) to cover opening the structured hook editor, persisting a shell-hook command, and rendering the overlay itself. Updated [`README.md`](README.md) and [`WORK.md`](WORK.md) to reflect that raw JSON is no longer the primary hook-authoring path.
- Why: once the main edit surface moved into overlays, leaving hook bundles as literal JSON blobs would have kept the ugliest part of the system untouched. This turns hook authoring into the same picker/form model as the rest of setup instead of making hooks the one place where users still have to think in serialized arrays.

### 2026-05-04 — Finished the hook-editor ergonomics so the structured path is actually usable
- Tightened the hook overlay in [`internal/tui/update_agent_manage.go`](internal/tui/update_agent_manage.go), [`internal/tui/view_agent_manage.go`](internal/tui/view_agent_manage.go), and [`internal/tui/view.go`](internal/tui/view.go) with the missing row operations and discoverability: `a` adds a hook row, `d` deletes, `D` duplicates, and `[` / `]` reorder rows while the left-side hook list is focused.
- Improved hook-row labeling so prompt/shell rows read as actual operator-facing entries instead of anonymous blobs, and added in-overlay help for enum-style hook fields plus small command/body examples where the structured editor still benefits from guidance.
- Added a focused regression in [`internal/tui/update_agent_test.go`](internal/tui/update_agent_test.go) for reordering/duplicating hook rows, and updated [`README.md`](README.md) / [`WORK.md`](WORK.md) so the documented path now matches the finished behavior rather than the earlier MVP of the overlay.
- Why: the structured hook editor was already better than raw JSON, but without row operations it still felt half-finished. This closes the biggest remaining “can’t actually manage a list cleanly” gap.

### 2026-05-04 — New Run now defaults to the primary path and advanced config is framed more honestly
- Reworked the Agent launch composer in [`internal/tui/update_agent_launch.go`](internal/tui/update_agent_launch.go), [`internal/tui/view_agent_launch.go`](internal/tui/view_agent_launch.go), and [`internal/tui/model.go`](internal/tui/model.go) so the default run path is now `Role -> Engine -> Repo(if needed) -> Note -> Review`, `enter` advances that path instead of launching early, and per-run `Prompt` / `Hooks` / `Perms` overrides stay hidden unless explicitly expanded with `a` or already active.
- Reframed in-app setup/editing copy across [`internal/tui/view_agent_manage.go`](internal/tui/view_agent_manage.go), [`internal/tui/view_agent.go`](internal/tui/view_agent.go), and [`internal/tui/view.go`](internal/tui/view.go) so `Agent Setup` now reads as `Advanced Setup`, with clearer `Roles / Prompts / Hook Bundles / Engines` language and help/footer text that matches the calmer launch flow.
- Tightened attached-run wording in [`internal/tui/view_agent_attached.go`](internal/tui/view_agent_attached.go) and [`internal/tui/update_agent_attached.go`](internal/tui/update_agent_attached.go) so tmux and exec-chat surfaces both frame the object as a `run`, reduce runner-specific jargon, and make the live/read-only tmux distinction read more directly.
- Cleared the trust issues called out in the repo review: fixed the stale status-bar test expectation in [`internal/statusbar/tmux_test.go`](internal/statusbar/tmux_test.go), removed dead README links to missing design docs, and updated the Agents/Advanced Setup workflow docs in [`README.md`](README.md).
- Updated focused regressions in [`internal/tui/view_agent_test.go`](internal/tui/view_agent_test.go) to match the new launch/setup copy, and `GOCACHE=/tmp/go-build go test ./...` now passes.
- Why: the biggest remaining product problem was not missing capability, it was that the common Agents path still felt like operating internal UI state. This pass makes the default workflow more linear and pushes the library/editing machinery into a more explicitly advanced area without ripping out the underlying features.
...
- Why: the config-level blacklist existed but discovery still loaded blocked files, and the user-facing config example had drifted behind the actual supported schema.

### 2026-05-01 — Agents dashboard header cleanup + bounded right peek on short terminals
- Reworked the Agents list header again after trying the denser chip/telemetry version: it now stays intentionally minimal and shows only the active filter plus Foreman on/off state. Files: [`internal/tui/view_agent.go`](internal/tui/view_agent.go), [`internal/tui/view_agent_test.go`](internal/tui/view_agent_test.go), [`README.md`](README.md), [`WORK.md`](WORK.md).
- Tightened the right-hand peek summary so repo/task metadata wraps inside the pane width, long task/note fields no longer force ugly spillover, and short terminals always retain a real scroll window for transcript/log output instead of losing the pane to wrapped header content. Files: [`internal/tui/view_agent.go`](internal/tui/view_agent.go).
- Reworked attached exec-chat jobs (including Ollama) away from the old supervision split and toward a real in-app chat surface: non-tmux jobs now render as a full-width transcript + boxed composer view without the extra outer panel, with transcript paging kept available while the composer is focused. Live exec jobs also open straight into the composer by default. Tmux-backed jobs keep the existing rail/log layout. Files: [`internal/tui/view_agent_attached.go`](internal/tui/view_agent_attached.go), [`internal/tui/update_agent_attached.go`](internal/tui/update_agent_attached.go), [`README.md`](README.md).
- Why: the old top-of-left-panel treatment felt noisy and the right pane could become visually taller than the list on shorter terminals because metadata wrapping consumed the available height before the output tail had room to scroll.

### 2026-05-01 — Retry split into immediate rerun (`r`) and editable relaunch (`R`)
- Added a real immediate retry path on lowercase `r`: it now relaunches the selected job with the same repo, note, and executor override, but always as an attended immediate run instead of re-queuing it through Foreman. This unblocks rerunning jobs that previously died on pre-hook or dirty-tree conditions once the repo is clean again, and avoids the confusing "retry just waits in queue again" behavior for Foreman jobs. Files: [`internal/cockpit/manager.go`](internal/cockpit/manager.go), [`internal/cockpit/manager_test.go`](internal/cockpit/manager_test.go), [`internal/tui/update_agent_launch.go`](internal/tui/update_agent_launch.go), [`internal/tui/update_agent.go`](internal/tui/update_agent.go), [`internal/tui/update_agent_attached.go`](internal/tui/update_agent_attached.go), [`internal/tui/update_agent_test.go`](internal/tui/update_agent_test.go), [`internal/tui/view_agent_test.go`](internal/tui/view_agent_test.go).
- Repurposed uppercase `R` to preload the New Run composer from an existing job so retry becomes editable: role/provider/repo/note/Foreman mode come back populated and you land in the composer ready to tweak before relaunching. Updated user docs and help text accordingly. Files: [`internal/tui/view.go`](internal/tui/view.go), [`README.md`](README.md), [`WORK.md`](WORK.md).
- Why: the old `R` behavior mostly reopened an existing session, which was redundant with `enter`/`i` and did not match the backlog's "retry" expectation. Splitting "rerun exactly" from "rerun after edits" makes both paths explicit.

### 2026-05-01 — New Run header now shows immediate vs Foreman queue mode
- The Agent launch composer title line now includes an explicit mode badge: `[Start now]` for immediate launches and `[Foreman queue]` when `ctrl+t` parks the run for Foreman. This keeps the current launch mode visible even after you tab away from the brief/review area where the secondary status copy used to be easier to miss. Files: [`internal/tui/view_agent_launch.go`](internal/tui/view_agent_launch.go), [`internal/tui/view_agent_test.go`](internal/tui/view_agent_test.go), [`README.md`](README.md), [`WORK.md`](WORK.md).
- Why: the queue-only toggle already worked, but the mode state was easy to lose in the composer; surfacing it in the header turns it into persistent context instead of a one-line detail lower in the panel.

### 2026-05-01 — Claude perms enum now actually enforces read-only / scoped-write
- `claudePermissionMode` previously emitted no `--permission-mode` flag for `read-only` or `scoped-write` jobs, which meant interactive Claude runs ignored those policies entirely (the agent could still write files; it would just prompt). Now: `read-only` → `--permission-mode plan`, `scoped-write` → `--permission-mode acceptEdits`, `wide-open` unchanged at `bypassPermissions`. Unattended foreman runs map `read-only` to `plan` as well so queued analyzers can't slip into edits. Files: [`internal/cockpit/manager.go`](internal/cockpit/manager.go), [`internal/cockpit/manager_test.go`](internal/cockpit/manager_test.go).
- Why: the perms enum was load-bearing in presets but two of three values were silent no-ops for Claude. Codex was already doing the right thing via `--sandbox`.

### 2026-05-01 — Per-launch role/prompt/hooks/perms overrides + provider model wiring
- New Run composer gained three override sub-tabs: **Prompt · Hooks · Perms**. Each defaults to "(role)" and lets the operator swap a single piece of the role's recipe for one launch without editing or duplicating a preset. Overrides fold into a one-off `LaunchPreset` at submit time via `effectiveLaunchPreset`. Switching role resets all overrides. Files: [`internal/tui/model.go`](internal/tui/model.go), [`internal/tui/update_agent_launch.go`](internal/tui/update_agent_launch.go), [`internal/tui/view_agent_launch.go`](internal/tui/view_agent_launch.go), [`internal/tui/update_agent.go`](internal/tui/update_agent.go), [`internal/tui/update_agent_picker.go`](internal/tui/update_agent_picker.go).
- Manage screen reworked from a one-line cycle hint into a real tab strip: **Roles (1) · Prompts (2) · Hooks (3) · Engines (4)** at the top, with direct number hotkeys jumping straight to a kind in addition to `[`/`]` cycling. Files: [`internal/tui/view_agent_manage.go`](internal/tui/view_agent_manage.go), [`internal/tui/update_agent_manage.go`](internal/tui/update_agent_manage.go).
- Wired `ExecutorSpec.Model` into the claude and codex command builders. Provider profiles can now set `executor.model: "claude-sonnet-4-6"` (or `claude-opus-4-7`, `gpt-5`, etc.) directly instead of stuffing `--model` into Args. A user-supplied `--model` in Args still wins so explicit overrides keep working. Files: [`internal/cockpit/manager.go`](internal/cockpit/manager.go), [`internal/cockpit/manager_test.go`](internal/cockpit/manager_test.go).
- Dropped the hard-coded `## FOREMAN PROTOCOL` prompt block. The `Permissions` enum already drives the real `--permission-mode` flag; the prompt sentence was advisory and the model could ignore it (and did). Foreman behaviour now flows entirely through preset perms + the supervisor markers. Files: [`internal/cockpit/hooks.go`](internal/cockpit/hooks.go), [`internal/cockpit/presets_test.go`](internal/cockpit/presets_test.go).
- Why: launching agents was bunk for a power-user — to mix senior-dev's prompt with pm's hook bundle and read-only perms required either editing a preset by hand or making a new preset. The data model already supported all the primitives; only the launch-time and manage-time UX couldn't compose them.
...
## 2026-05-01 - attached exec-chat bottom anchoring
